...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores = h2.dot(W3) + b3
        # h1_output = np.maximum(0, X.dot(W1)+b1) # ReLU 激活函数
        # h2_output = h1_output.dot(W2)+b2
        # scores = h2_output.dot(W3)+b3
        ...
        # 之前的：
        # print("scors_shape是"+str(scores.shape))
        # shift_scores = scores - np.max(scores, axis=1).reshape((-1, 1))
        # print("shift_scors_shape是"+str(shift_scores.shape))
        # softmax_output = np.exp(shift_scores) / np.sum(np.e(shift_scores), axis=1).reshape(-1,1)
        #
        ...
        grads['W3'] = h2.T.dot(delta3) / h2.shape[0]
        grads['W3'] += 2 * reg * W3
        grads['b3'] = np.ones(N).dot(delta3) / h2.shape[0]

        grads['W2'] = np.zeros_like(W2)
        delta2 = delta3.dot(W3.T) * h1
        ...
        # dscores /= N
        # grads['W3'] = h2_output.T.dot(dscores) + reg * W3
        # grads['b3'] = np.sum(dscores, axis=0)
        #
        # dh2 = dscores.dot(W3.T)
        # dscores[range(N), list(y)] -= 1
        # dh2 /= N
        # grads['W2'] = h1_output.T.dot(dh2) + reg * W2
